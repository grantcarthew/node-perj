...
test(title, (t) => {
  for (const level of Object.keys(log.levels)) {
    ...
    t.test(`${titlePrefix}: two messages two data mixed order`, (t) => {
      ...
      t.equal(
        Object.keys(tool.jsonOut).length,
        5,
        "jsonOut should have 5 keys"
      );
      t.equal(Object.keys(tool.objOut).length, 5, "objOut should have 5 keys");
      t.equal(
        tool.getType(tool.jsonOut.time),
        "Number",
        "jsonOut time should be a number"
      );
      t.equal(
        tool.getType(tool.objOut.time),
        "Number",
        "objOut time should be a number"
      );
      t.equal(tool.jsonOut.level, level, "jsonOut level should match");
      t.equal(tool.objOut.level, level, "objOut level should match");
      t.equal(
        tool.getType(tool.jsonOut.msg),
        "String",
        "jsonOut msg should be a string"
      );
      t.equal(
        tool.getType(tool.objOut.msg),
        "String",
        "objOut msg should be a string"
      );
      t.equal(tool.jsonOut.msg, data.msg[1], "jsonOut msg should match");
      t.equal(tool.objOut.msg, data.msg[1], "objOut msg should match");
      t.equal(
        tool.getType(tool.jsonOut.data),
        "Array",
        "jsonOut data should be an array"
      );
      t.equal(
        tool.getType(tool.objOut.data),
        "Array",
        "objOut data should be an array"
      );
      t.equal(
        tool.jsonOut.data.length,
        3,
        "jsonOut data array length should be 3"
      );
      t.equal(
        tool.objOut.data.length,
        3,
        "objOut data array length should be 3"
      );
      t.deepEqual(
        tool.jsonOut.data[0],
        data.tardis,
        "jsonOut data[0] should match tardis"
      );
      t.deepEqual(
        tool.objOut.data[0],
        data.tardis,
        "objOut data[0] should match tardis"
      );
      t.deepEqual(
        data.tardis,
        tool.jsonOut.data[0],
        "tardis should match jsonOut data[0]"
      );
      t.deepEqual(
        data.tardis,
        tool.objOut.data[0],
        "tardis should match objOut data[0]"
      );
      t.deepEqual(
        tool.jsonOut.data[1],
        data.serenity,
        "jsonOut data[1] should match serenity"
      );
      t.deepEqual(
        tool.objOut.data[1],
        data.serenity,
        "objOut data[1] should match serenity"
      );
      t.deepEqual(
        data.serenity,
        tool.jsonOut.data[1],
        "serenity should match jsonOut data[1]"
      );
      t.deepEqual(
        data.serenity,
        tool.objOut.data[1],
        "serenity should match objOut data[1]"
      );
      t.equal(
        tool.jsonOut.data[2],
        data.msg[0],
        "jsonOut data[2] should match msg[0]"
      );
      t.equal(
        tool.objOut.data[2],
        data.msg[0],
        "objOut data[2] should match msg[0]"
      );
      t.end();
    });
